...
__global__ void createRandScene(Hittable** list, Hittable** world, Camera** camera, curandState* randState)
{
	curand_init(2019, 0, 0, randState);
	curandState localRandState = *randState;

	int i = 0;
	list[i++] = new Sphere(Vec3(0, -1000.0, -1), 1000, new Lambertian(Vec3(0.5, 0.5, 0.5)));
	...
	for (int a = -11; a < 11; a++) {
		for (int b = -11; b < 11; b++) {
			...
			Vec3 center(RND + a, radius, RND + b);
			if (choose_mat < 0.4f) {
	...
	*world = new HittableList(list, 22*22+5);
